...
<div class=" card px-4 py-4 container my-5   border-1 border-opacity-10 rounded-4">
    ...
    <h2 class="text-black fw-semibold">Buat Daftar Barang</h2>
        ...
        <form  method="POST" class="row d-flex" action="{{url("inventaris/$inventaris->id")}}" >
         @csrf
         @method('PATCH')

            <div class="col-3">
                <input type="text" class="form-control p-2" id="name" placeholder="Nama Barang"  name="nama_barang" value="{{$inventaris->nama_barang}}" required>
            </div>
            <div class="col-2">
                <td><select class="form-select" id="ruangan" name="ruangan" value="{{$inventaris->ruangan}}" aria-label="Default select example" required>
            ...
            <div class="col-1 mb-5">
                <input class="form-control p-2" id="jumlah_barang" rows="3" placeholder="Jumlah" name="jumlah_barang" value="{{$inventaris->jumlah_barang}}" required></input>
            </div>
            <div class="col-6 mb-5">
                <input class="form-control p-2" id="info_barang" rows="3" placeholder="Deskripsi" name="info_barang" value="{{$inventaris->info_barang}}" required></input>
            </div>

            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-warning mb-3 px-3 text-white px-5">Simpan ubahan</button>
